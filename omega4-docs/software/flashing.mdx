...
description: Enter recovery flashing mode on Omega4 and write factory images to all partitions using rkdeveloptool.
---

import { GiscusDocComment } from '/src/components/GiscusComment';

Use this procedure when you need to reflash an Omega4 completely (bootloader, kernel, and rootfs). It assumes you have the official image set for your hardware revision and a host machine with `rkdeveloptool` available. The tool is published at https://github.com/rockchip-linux/rkdeveloptool and official firmware builds are published at https://github.com/OnionIoT/openwrt-omega4/releases.
...
rkdeveloptool wl 0x0    env.img
rkdeveloptool wl 0xa00  uboot.img
rkdeveloptool wl 0x1200 boot.img
rkdeveloptool wl 0x5200 -1 rootfs.img
rkdeveloptool rd
```
...
Erase the flash before reflashing the rootfs to clear stale metadata:
...
Then rerun the flashing commands from the table above. This forces a clean UBI attach on the next boot.
