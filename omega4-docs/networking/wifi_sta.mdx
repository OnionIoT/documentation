...
For a setup where the Omega4 joins an upstream Wi-Fi network on one band and hosts an AP on the other band, see [Cross-Band Wi-Fi STA + AP](./wifi-cross-band-sta-ap).
...
- Standard UCI config uses one Wi-Fi role at a time. To switch back to AP mode, restore `mode='ap'` and your AP network, for example `network='wlan'`.
