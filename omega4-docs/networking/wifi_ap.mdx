...
## 4. Add `wlan` to the LAN firewall zone
Let Wi‑Fi clients reach LAN services and any upstream WAN by adding the new `wlan` interface to the existing `lan` zone:
```bash
uci add_list firewall.lan.network='wlan'
uci commit firewall
/etc/init.d/firewall restart
```
Check with `uci show firewall.lan.network` if you want to confirm the interface list (duplicates are harmless but you can dedupe by editing the UCI list).
...
## 6. Force an 802.11n/HT40 channel
...
## 9. Troubleshooting (AP interface not up)
...
- Standard UCI config uses one Wi-Fi role at a time. If STA mode is enabled, the AP interface may be disabled; set `wireless.default_radio0.mode='ap'` and reload. For STA on one band plus AP on the other band, see [Cross-Band Wi-Fi STA + AP](./wifi-cross-band-sta-ap).
