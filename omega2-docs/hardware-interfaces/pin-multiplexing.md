...
To make the pin multiplexing configuration permanent we can configure the omega to run the gpio mux command automatically at boot time. For more information, see the article on [running a command on boot](/omega2-docs/software/running-command-on-boot). 
